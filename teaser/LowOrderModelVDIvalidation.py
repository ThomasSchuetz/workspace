...
#%% partialReducedOrderModel                                          
    ...
    if testcase == True:
        ...
        (R1i, C1i, Ai, RRest, R1o, C1o, Ao, RWin, Aw, A_win_tot,
         ...
         splitfac, epsw, g, C_HC, Tig, model) = valid.testCase6(houseData,n,timesteps,model,testcase)
...
#%% calculate solar input for reduced order model and for eqAirTemp through sunblinds
    ...
    sunblindsig2 = np.zeros((n,T))

    for i in range(n):
        sunblindsig2[i,solarBeforeSunblinds[i,:] > I_max] = 1 - gsunblind[i]
        afterSunblind[i,solarBeforeSunblinds[i,:] > I_max] = solarBeforeSunblinds[i,solarBeforeSunblinds[i,:] > I_max] * gsunblind[i]
    ...
    weighted_sum = np.dot(Aw, afterSunblind) / temp
...
#%% 
    ...
    with open("inputs_eqAir.pkl", "wb") as fout:
        pkl.dump(raw_inputs, fout, pkl.HIGHEST_PROTOCOL)
        pkl.dump(houseData, fout, pkl.HIGHEST_PROTOCOL)
        pkl.dump(solarRad_in, fout, pkl.HIGHEST_PROTOCOL)
        pkl.dump(sunblindsig, fout, pkl.HIGHEST_PROTOCOL)
