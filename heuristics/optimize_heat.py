...
def optimize(max_emissions, options, fixed_components):
    # Constants
    ...
    if options["opt_costs"]:
        model.setObjective(sum(weight_days[d] * 
                               sum(Q_HC[d,t] for t in range(timesteps))
                           for d in range(number_days)), gp.GRB.MINIMIZE)
    else:
