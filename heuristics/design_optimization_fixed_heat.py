...
def optimize(max_emissions, options, Q_HC, fixed_components):
    # Constants
    ...
    for d in range(number_days):
        ...
        # Heat demand
            Q_HC_dummy[d,t] = model.addVar(vtype="C", name="Q_HC_"+timetag,
    ...
    for d in range(number_days):
        ...
        # Using T_i instead of T_op for temperature control
            ...
            model.addConstr(-H_tr_is * T_s[d,t] + H_ve * T_i[d,t] + H_tr_is * T_i[d,t] - 1000 * Q_HC_dummy[d,t] ==
    ...
    model.addConstr(x["EH"] + sum(x["Boiler",i] for i in range(number_devices["Boiler"])) <= 1, "Boiler or EH")
    model.addConstr(x["EH"] + sum(x["CHP",i] for i in range(number_devices["CHP"])) <= 1, "CHP or EH")
    ...
    # Fixed components
    for i in range(4):
        model.addConstr(x["opaque", "wall", i] == fixed_components["walls"][i])
        model.addConstr(x["opaque", "roof", i] == fixed_components["roofs"][i])
        model.addConstr(x["window", i] == fixed_components["windows"][i])
    ...
    if model.status == gp.GRB.OPTIMAL or model.status == gp.GRB.TIME_LIMIT:
        ...
        #5R1C ISO 13790 
        ###############
        res_Q_HC = np.array([[Q_HC_dummy[d,t].X for t in range(timesteps)] for d in range(number_days)])
    ...
    with open(options["filename_results"], "wb") as fout:
        ...
        pickle.dump(Q_HC, fout, pickle.HIGHEST_PROTOCOL)
